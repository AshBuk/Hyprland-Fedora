...
# Build with Clang (upstream toolchain); GCC miscompiles 0.56.0 Desktop::View
# base-class construction -> null vtable -> layer-surface SIGSEGV. LTO off too.
...
# Build everything with Clang (upstream toolchain; GCC miscompiles Desktop::View).
export CC=clang
export CXX=clang++

# RPM standard optflags (Clang-safe via `%%global toolchain clang`).
# Passed explicitly to cmake - env vars don't work reliably in mock chroot.
...
# 6) aquamarine (Clang compiles the generated zero-size-array protocol code fine)
...
cmake -B build \
  ...
  -Dhyprwayland-scanner_DIR="$VENDOR_PREFIX/lib64/cmake/hyprwayland-scanner" \
  -DCMAKE_CXX_FLAGS="$BUILD_CXXFLAGS -I$VENDOR_PREFIX/include" \
  -DBUILD_TESTING=OFF \
...
%changelog
* Thu Jul 23 2026 Asher Buk <AshBuk@users.noreply.github.com> - 0.56.0-2
- Update to Hyprland 0.56.0, Aquamarine 0.12.0 -> 0.13.0
- Bump glaze 7.0.0 -> 7.2.0 to match upstream's tested build
- Disable LTO, Build with Clang (GCC miscompiles Desktop::View -> layer-surface crash)
