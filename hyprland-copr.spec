...
# Use CXXFLAGS env var to APPEND -fpermissive (not replace default flags)
# Using -DCMAKE_CXX_FLAGS=... would REPLACE all flags and break include paths
export CXXFLAGS="${CXXFLAGS:+$CXXFLAGS }-fpermissive"
...
# 6) aquamarine (CXXFLAGS with -fpermissive is inherited from env)
...
# 8) Hyprland (CXXFLAGS with -fpermissive is inherited from env)
cmake -B build \
